...
APPL = Flask(__name__)
APPL.config['SQLALCHEMY_DATABASE_URI'] = 'mysql://root:12345@127.0.0.1:3306/db1'
APPL.config['SECRET_KEY'] = 'shalini'
APPL.config['SQLALCHEMY_TRACK_MODIFICATIONS'] = True
...
@APPL.route('/delete/<string:s_id>', methods=['POST'])
def delete(s_id):
...
@APPL.route('/db_edit', methods=['POST'])
def db_edit():
...
@APPL.route('/db_student', methods=["GET", "POST"])
def db_student():
...
@APPL.route('/db_class', methods=["GET", "POST"])
def db_class():
